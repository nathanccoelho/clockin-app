...
export default function Login({ onLogin }) {
  const [tela, setTela] = useState('login') // login | cadastro | recuperar | codigo | nova_senha
  ...
  async function enviarCodigoRecuperacao() {
    ...
    const { data: colab } = await supabase
      ...
      .eq('email', emailRecuperar.trim().toLowerCase())
      .eq('status', 'aprovado')
      .maybeSingle()
    ...
    setColaboradorRecuperar(colab)

    try {
      await supabase.functions.invoke('enviar-codigo-email', {
        body: { email: colab.email, nome: colab.nome, codigo, tipo: 'recuperacao' }
      })
    } catch {
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h2 className="text-white text-2xl font-bold mb-1">Nova senha</h2>
          <p className="text-gray-400 text-sm">Digite sua nova senha</p>
        </div>
        <form onSubmit={salvarNovaSenha} className="space-y-4">
          <div className="relative">
            <input type={mostrarNovaSenha ? 'text' : 'password'} placeholder="Nova senha" value={novaSenha}
              onChange={e => setNovaSenha(e.target.value)}
              className="w-full bg-gray-800 text-white border border-gray-700 rounded-2xl p-4 pr-12 text-lg placeholder-gray-500 focus:border-green-500 focus:outline-none"
              required />
          ...
          <input type="password" placeholder="Confirmar nova senha" value={confirmarNovaSenha}
            onChange={e => setConfirmarNovaSenha(e.target.value)}
            className="w-full bg-gray-800 text-white border border-gray-700 rounded-2xl p-4 text-lg placeholder-gray-500 focus:border-green-500 focus:outline-none"
            required />
          {confirmarNovaSenha && novaSenha !== confirmarNovaSenha && (
            <p className="text-red-400 text-xs">⚠️ As senhas não coincidem</p>
          )}
  ...
  // ── TELA CÓDIGO RECUPERAÇÃO ──────────────────────────────────
  if (tela === 'codigo') return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <button onClick={() => setTela('recuperar')} className="text-gray-400 mb-6 w-auto bg-transparent border-0 shadow-none p-0 flex items-center gap-2 hover:text-white cursor-pointer">
          ← Voltar
        </button>
        <div className="text-center mb-8">
          <div className="text-5xl mb-4">📧</div>
          ...
          <p className="text-gray-400 text-sm">
            ...
            <span className="text-green-400 font-semibold">{emailRecuperar}</span>
          </p>
        </div>
        <div className="space-y-4">
          <input type="text" maxLength={6} placeholder="000000" value={codigoDigitado}
          ...
          <button onClick={verificarCodigoRecuperacao} disabled={codigoDigitado.length !== 6}
            className="w-full bg-green-500 text-white font-bold py-4 rounded-2xl text-lg hover:bg-green-600 disabled:opacity-50 cursor-pointer transition-colors">
            Verificar código
          </button>
          <div className="text-center">
            {reenvioTimer > 0 ? (
              ...
              <button onClick={enviarCodigoRecuperacao}
                className="text-green-400 text-sm font-semibold w-auto p-0 bg-transparent border-0 shadow-none cursor-pointer hover:text-green-300">
                Reenviar código
              </button>
            )}
          </div>
  ...
  if (tela === 'recuperar') return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <button onClick={() => { setTela('login'); setErro('') }} className="text-gray-400 mb-6 w-auto bg-transparent border-0 shadow-none p-0 flex items-center gap-2 hover:text-white cursor-pointer">
          ← Voltar ao login
        </button>
        <div className="text-center mb-8">
          <div className="text-5xl mb-4">🔒</div>
          <h2 className="text-white text-2xl font-bold mb-2">Recuperar acesso</h2>
          <p className="text-gray-400 text-sm">Digite o email cadastrado para receber o código de verificação</p>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-gray-400 text-sm mb-1">Email cadastrado</label>
            <input type="email" placeholder="seu@email.com" value={emailRecuperar}
              onChange={e => setEmailRecuperar(e.target.value)}
              className="w-full bg-gray-800 text-white border border-gray-700 rounded-2xl p-4 text-lg placeholder-gray-500 focus:border-green-500 focus:outline-none"
              required />
          </div>
          {erro && <p className="text-red-400 text-sm">{erro}</p>}
          <button onClick={enviarCodigoRecuperacao} disabled={loading || !emailRecuperar.trim()}
            className="w-full bg-green-500 text-white font-bold py-4 rounded-2xl text-lg hover:bg-green-600 disabled:opacity-50 cursor-pointer transition-colors">
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <h1 className="text-white text-3xl font-bold">Ponto Eletrônico</h1>
          <p className="text-gray-400 text-sm mt-1">Faça login para continuar</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-gray-400 text-sm mb-1">CPF ou E-mail</label>
            <input type="text" placeholder="Digite seu CPF ou e-mail" value={login}
              onChange={e => setLogin(e.target.value)}
              className="w-full bg-gray-800 text-white border border-gray-700 rounded-2xl p-4 text-lg placeholder-gray-500 focus:border-green-500 focus:outline-none cursor-text"
              required />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-green-500 text-white font-bold py-4 rounded-2xl text-lg hover:bg-green-600 disabled:opacity-50 cursor-pointer transition-colors mt-2">
        ...
        <p className="text-center text-gray-500 mt-6 text-sm">
          ...
          <button onClick={() => setTela('cadastro')}
            className="text-green-400 font-semibold w-auto p-0 bg-transparent border-0 shadow-none cursor-pointer hover:text-green-300">
            Cadastre-se
          </button>
        </p>
        <p className="text-center text-gray-600 mt-3 text-xs">
          Esqueceu a senha?{' '}
          <button onClick={() => { setTela('recuperar'); setErro(''); setEmailRecuperar('') }}
            className="text-gray-400 w-auto p-0 bg-transparent border-0 shadow-none cursor-pointer hover:text-gray-300">
            Recuperar acesso
          </button>
        </p>
      </div>
